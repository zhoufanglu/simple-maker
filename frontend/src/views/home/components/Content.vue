...
<template>
  <div v-auto-animate class="content">
    <Draggable v-auto-animate style="height: auto" handle=".handle" ghost-class="ghost" animation="200"
      :list="rankingRows" item-key="levelName">
      <template #item="{ element: rank, index }">
        <ul class="rank-row">
          ...
          }" class="img-row" ghost-class="ghost" animation="200" :list="rank.items" group="img-row" itemKey="path">
            <template #item="{ element: img, index: imgIndex }">
              <n-image @dblclick="dbRemoveClick(img, index, imgIndex)" :src="img.path" preview-disabled width="80"
                class="img-item">
              </n-image>
            </template>
          </draggable>
          ...
          <Motion v-if="homeStore.modeType === 'edit'">
            <li class="action-item drag-handle">
              ...
              <i class="iconfont fa fa-align-justify handle" @click="handleDelRow(index)">&#xe616;</i>
            </li>
          </Motion>
        </ul>
      </template>
